...
export class TestHarvesting {
  static async test() {
    ...
    // let categories = rawData.children; //Getting all BASIC categories of willys
      ...
      let categoriesOfMathem = await rawDataMathem.products; // Mathem Products
 ...
 // let mathemData = await MathemHarvester.getAllProducts(cat);
  ...
  let allProductsOfMathem = await MathemHarvester.overWriteProducts(categoriesOfMathem)
    
   console.log(categoriesOfMathem.length);
